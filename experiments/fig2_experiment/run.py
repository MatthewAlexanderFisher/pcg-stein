...
def parse_args():
    ...
    p.add_argument(
        ...
        type=bool,
        default=False,
        help="Use CPU or GPU boolean (defaults to False: use GPU)",
    )
    p.add_argument(
        "-c",
        "--config",
...
from pcg_stein.pcg import _pcg_core_history_static_mats
from pcg_stein.kernel import LinearOperator
...
def process_one_rep(args_tuple):
    ...
    for lengthscale in lengthscale_pbar:
        ...
        stein_mv = partial(kernel.stein_matvec,
                           X=X_h, 
                           Sx=Sx_h, 
                           lengthscale=lengthscale
                           )
        
        K = LinearOperator(stein_mv, stein_mv, n=NUM_MCMC_SAMPLES)

        b = jnp.ones(NUM_MCMC_SAMPLES)  # i.e. shape (X_h.shape[0], )

        # perform reference CG
        x_cg, m_cg, res_cg, wce_cg = _pcg_core_history_static_mats(
            K, b, rtol=0.0, atol=0.0, wce_tol=0.0, maxiter=MAXITER
        )

        for i in range(wce_cg.shape[0]):
            cg_long_rows.append(
                {
                    "replicate": rep,
                    "lengthscale": lengthscale,
                    "iteration": i,
                    "value": float(wce_cg[i]),
                    "method": "cg",
                }
            )
        ...
        for precon in precon_pbar:
            ...
            for precon_arg in precon_args_pbar:
                ...
                precon_mat = precon(precon_key, K, full_mat=False, **precon_kwargs)
                print(precon_mat)
                # perform PCG
                x_pcg, m_pcg, res_pcg, wce_pcg = _pcg_core_history_static_mats(
                    K,
                    b,
                    rtol=0.0,
                    atol=0.0,
                ...
                for i in range(wce_pcg.shape[0]):
...
for rep in mcmc_pbar:
    ...
    # Copy to device
    X_rep = jax.device_put(X_rep)
    Sx_rep = jax.device_put(Sx_rep)
